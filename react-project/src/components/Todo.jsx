...
function Todo({ todo, todos, searchCriteria, setTodos, indexUpdateTask, setIndexUpdateTask, setFilteredTodos }) {
    ...
    const updateDB = (todo) => {
        ...
        fetch(`http://localhost:3000/todos/${todo.id}`, requestOptions)
            .then(res => res.json())
            .then(data => {
                const updateTodos = todos.map(currentTodo => data.id == currentTodo.id ? data : currentTodo);
